...
#define OBJECT_TRACKER_METHOD(METHOD_NAME) \
  Java_com_test_testing_tracking_ObjectTracker_##METHOD_NAME  // NOLINT
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(initNative)(JNIEnv
    * env,
    jobject thiz,
            jint
    width,
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(releaseMemoryNative)(JNIEnv
    * env,
    jobject thiz
    );
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(registerNewObjectWithAppearanceNative)(
    JNIEnv
    * env,
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(setPreviousPositionNative)(
    JNIEnv
    * env,
    jobject thiz, jstring
    object_id,
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(setCurrentPositionNative)(
    JNIEnv
    * env,
    jobject thiz, jstring
    object_id,
    ...
    JNIEXPORT
            jboolean
    JNICALL OBJECT_TRACKER_METHOD(haveObject)(JNIEnv * env, jobject
    thiz,
    jstring object_id
    );
    ...
    JNIEXPORT
            jboolean
    JNICALL OBJECT_TRACKER_METHOD(isObjectVisible)(JNIEnv * env,
                                                   jobject
    thiz,
    jstring object_id
    );
    ...
    JNIEXPORT
            jstring
    JNICALL OBJECT_TRACKER_METHOD(getModelIdNative)(JNIEnv * env,
                                                    jobject
    thiz,
    jstring object_id
    );
    ...
    JNIEXPORT
            jfloat
    JNICALL OBJECT_TRACKER_METHOD(getCurrentCorrelation)(JNIEnv * env,
                                                         jobject
    thiz,
    jstring object_id
    );
    ...
    JNIEXPORT
            jfloat
    JNICALL OBJECT_TRACKER_METHOD(getMatchScore)(JNIEnv * env, jobject
    thiz,
    jstring object_id
    );
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(getTrackedPositionNative)(
    JNIEnv
    * env,
    jobject thiz, jstring
    object_id,
    jfloatArray rect_array
    );

    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(nextFrameNative)(JNIEnv
    * env,
    jobject thiz,
            jbyteArray
    y_data,
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(forgetNative)(JNIEnv
    * env,
    jobject thiz,
            jstring
    object_id);
    ...
    JNIEXPORT
            jbyteArray
    JNICALL OBJECT_TRACKER_METHOD(getKeypointsPacked)(
            JNIEnv * env, jobject
    thiz,
    jfloat scale_factor
    );
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(getCurrentPositionNative)(
    JNIEnv
    * env,
    jobject thiz, jlong
    timestamp,
    ...
    JNIEXPORT
    void JNICALL
    OBJECT_TRACKER_METHOD(drawNative)(JNIEnv
    * env,
    jobject obj,
            jint
    view_width,
    ...
    OBJECT_TRACKER_METHOD(downsampleImageNative)(
    JNIEnv
    * env,
    jobject thiz, jint
    width,
    ...
    * env,
    ...
    LOGI("Initialized!"); CHECK_ALWAYS(get_object_tracker(env, thiz) == tracker,
...
RegisterNewObjectWithAppearance(
        id_str,
reinterpret_cast
<const uint8_t *>(pixels), bounding_box
);

env->
ReleaseByteArrayElements(frame_data, pixels, JNI_ABORT
);

env->
ReleaseStringUTFChars(object_id, id_str
);
}
...
bounding_box.CopyToArray(reinterpret_cast
<float *>(rect)
);
env->
ReleaseFloatArrayElements(rect_array, rect,
0);
}
...
int i = 0;
i < 6; ++i) {
vision_gyro_matrix_array[i] = static_cast
<float>(jmat[i]);
}
}
// Copy image into currFrame.
...
env->
ReleaseByteArrayElements(y_data, pixels, JNI_ABORT
);

if (uv_data != NULL) {
env->
ReleaseByteArrayElements(uv_data, uv_pixels, JNI_ABORT
);
}

if (vg_matrix_2x3 != NULL) {
env->
ReleaseFloatArrayElements(vg_matrix_2x3, jmat, JNI_ABORT
);
}
...
env->
SetFloatArrayRegion(keypoints,
0,
number_of_keypoints *kKeypointStep,
        keypoint_arr
);
...
SetByteArrayRegion(
        keypoints,
0,
number_of_keypoints *bytes_per_keypoint, keypoint_arr
);
...
new_position.
CopyToArray(point_arr);
env->
SetFloatArrayRegion(delta,
0, 4, point_arr);
}

JNIEXPORT
void JNICALL
OBJECT_TRACKER_METHOD(drawNative)(
JNIEnv
* env,
jobject thiz, jint
view_width,
...
object_tracker->
Draw(view_width, view_height, frame_to_canvas
);
env->
ReleaseFloatArrayElements(frame_to_canvas_arr, frame_to_canvas,
        JNI_ABORT
);
}
}
...
env->
ReleaseByteArrayElements(input, input_array, JNI_ABORT
);
env->
ReleaseByteArrayElements(output, output_array,
0);
}
